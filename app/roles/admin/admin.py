...
from app.database.user_db_operations import (
    ...
    get_user_by_id, get_user_by_telegram_tag,
)
...
@admin.message(F.text == labels.MANAGE_ADMINS)
...
@admin.callback_query(F.data == Callbacks.add_admin_callback)
async def handle_add_admin_callback(callback: CallbackQuery, state: FSMContext):
    await on_add_admin_clicked(callback, state)


@admin.message(AdminManagementStates.waiting_for_admin_username)
...
@admin.callback_query(F.data.startswith("admin_clicked"))
...
@admin.callback_query(F.data.startswith(Callbacks.admin_delete_callback))
...
@admin.callback_query(F.data == Callbacks.return_to_admin_list_callback)
